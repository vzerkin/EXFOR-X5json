...
def get_cvar4fam(obj_c5data,fam):
    for str1 in obj_c5data:
        if (obj_c5data[str1]['fam']==fam): return obj_c5data[str1]['cvar']
    return None

def get_cvars(ds):
    obj_c5data=ds['c5data']
    lst=[]
    for str1 in obj_c5data:
        str2=str1+'|'+obj_c5data[str1]['fam']+'|'+obj_c5data[str1]['units']
        str2+='|'+obj_c5data[str1]['expansion']
        lst.append(str2)
    return lst
